...
@click.option("--input-type", default="mediawiki", type=click.Choice(["mediawiki"]), help="Type of the input data. The only current supported type is 'mediawiki'.")
@click.option("--output", "-o", type=click.File("w"), default=sys.stdout, help='Output file to write processed results in TSV format. Defaults to sys.stdout.')
@click.option('--additional-site', '-s', multiple=True, help='Additional sites to check, e.g. "wikidata.org"')
@click.option('--threshold-active', type=int, help="Last edits after the active threshold year will be marked as active.")
@click.option('--threshold-inactive', type=int, help="Last edits after the inactive threshold year but before the active threshold year will be marked as inactive.")
